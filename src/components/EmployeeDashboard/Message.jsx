...
const Message = () => {
  ...
  // Fetch all users when search modal opens
  useEffect(() => {
    if (!showSearchModal) {
      setAllUsers([]);
      setFilteredUsers([]);
    ...
    if (!companyId) {
      setSearchError('Company ID is missing. Cannot search users.');
      setSearchLoading(false);
      return;
    }
    ...
    const fetchUsers = async () => {
      try {
        ...
        console.log('DEBUG - Fetch all users response:', res.data);
        if (res.data.success) {
          const users = res.data.users.filter((u) => u._id !== user._id);
          setAllUsers(users);
          setFilteredUsers(users);
          if (users.length === 0) {
            setSearchError('No users found in your company.');
          }
        } else {
          setSearchError(res.data.error || 'Failed to fetch users.');
        }
      } catch (error) {
        ...
        if (error.message.includes('Network Error')) {
          ...
          setSearchError('An error occurred while fetching users. Please try again.');
        }
      } finally {
        setSearchLoading(false);
      }
      ...
      <div className="w-1/3 bg-gray-800 shadow-md border-r border-gray-700">
        <div className="p-4 border-b border-gray-700 flex justify-between items-center">
          ...
          <button
            ...
            className="bg-teal-600 hover:bg-teal-700 text-white px-3 py-1.5 rounded flex items-center text-sm transition-colors"
          >
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    {unreadCount > 0 && (
                      <span className="bg-teal-500 text-white text-xs rounded-full px-2 py-1">
                        {unreadCount}
                      </span>
                    )}
                  </div>
          ...
          <>
            ...
            >
              <input
                ...
                className="flex-1 p-2 bg-gray-700 text-gray-100 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
              <button
                type="submit"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg w-full max-w-md">
            ...
            <input
              ...
              placeholder="Search managers or colleagues..."
              className="w-full p-2 mb-4 bg-gray-700 text-gray-100 border border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
